-- =====================================================
-- TEXT SEARCH OPTIMIZATION - PHASE 2
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_components_model_sku_trgm
  ON "3.0_components" USING gin(model_sku gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_components_description_trgm
  ON "3.0_components" USING gin(description gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_components_brand_trgm
  ON "3.0_components" USING gin(brand gin_trgm_ops)
  WHERE brand IS NOT NULL;

COMMENT ON INDEX idx_components_model_sku_trgm IS 'CRITICAL: model_sku is searched in every /api/ask request';
COMMENT ON INDEX idx_components_description_trgm IS 'CRITICAL: description is searched in every /api/ask request';

-- Purchase Orders - PO number search
CREATE INDEX IF NOT EXISTS idx_purchases_po_number_trgm
  ON "6.0_purchases" USING gin(po_number gin_trgm_ops);
...
CREATE INDEX IF NOT EXISTS idx_proforma_pi_number_trgm
  ON "5.0_proforma_invoices" USING gin(pi_number gin_trgm_ops);
...
-- Components: Search across sku + description + brand
...
DROP INDEX IF EXISTS idx_components_model_sku_trgm;
DROP INDEX IF EXISTS idx_components_description_trgm;
...
DROP INDEX IF EXISTS idx_proforma_pi_number_trgm;
