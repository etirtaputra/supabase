                                    ...
                                    {isEditingComp ? (
                                      <div className="flex items-center gap-1.5">
                                        <div className="flex-1 min-w-0">
                                          <ComponentCombobox components={components} onSelect={(cid) => handleReassign('quote', item.quote_line_id, cid)} onCancel={() => setEditingItem(null)} />
                                        </div>
                                      ...
                                      <div className="flex items-start gap-1.5">
                                        <div className="min-w-0 flex-1">
                                          ...
                                          {isEditingLine && (
                                            <button onMouseDown={(e) => { e.preventDefault(); setEditingLine((prev) => prev && { ...prev, showCompSearch: true }); }} className="text-[10px] text-sky-400 hover:text-sky-300 transition-colors">change product</button>
                                          )}
                                        </div>
                                        {!isEditingLine && onUpdateQuoteItem && (
                                          <button onClick={() => setEditingItem({ type: 'quote', id: item.quote_line_id })} title="Re-assign component" className="opacity-0 group-hover:opacity-100 flex-shrink-0 mt-0.5 p-0.5 text-slate-600 hover:text-blue-400 transition-all">
                                      ...
                                      <div className="flex items-center justify-end gap-2">
                                        ...
                                        <div className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 flex-shrink-0 transition-opacity">
                                          {onUpdateQuoteLineItem && (
                                            <button onMouseDown={(e) => { e.preventDefault(); setEditingLine({ mode: 'edit', type: 'quote', id: item.quote_line_id, rowIdx: -1, targetId: 0, currency: item.currency, componentId: item.component_id ?? null, showCompSearch: false, qty: String(item.quantity), price: String(item.unit_price), saving: false }); }} title="Edit qty & price" className="p-0.5 text-sky-600 hover:text-sky-400 hover:bg-sky-500/10 rounded transition-all"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                                          )}
                                          {onDeleteQuoteLineItem && (
                                            <button onClick={async (e) => { e.stopPropagation(); if (!window.confirm('Delete this quote line item?')) return; await onDeleteQuoteLineItem(item.quote_line_id); }} title="Delete" className="p-0.5 text-red-700 hover:text-red-400 hover:bg-red-500/10 rounded transition-all"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                          )}
                                        </div>
                                      </div>
                                    )}
                                ...
                                <tr key={item.po_line_item_id} className={`border-b border-slate-800/40 last:border-0 group ${isEditingLine ? 'bg-emerald-500/5' : ''}`}>
                                  <td className="py-2 pr-4">
                                    {isEditingComp ? (
                                      <div className="flex items-center gap-1.5">
                                        <div className="flex-1 min-w-0">
                                          <ComponentCombobox components={components} onSelect={(cid) => handleReassign('po', item.po_line_item_id, cid)} onCancel={() => setEditingItem(null)} />
                                        </div>
                                        {editingSaving && <span className="text-[10px] text-slate-500 flex-shrink-0">saving…</span>}
                                      </div>
                                    ) : isEditingLine && editingLine?.showCompSearch ? (
                                      ...
                                      <div className="flex items-start gap-1.5">
                                        <div className="min-w-0 flex-1">
                                          ...
                                          {isEditingLine && (
                                            <button onMouseDown={(e) => { e.preventDefault(); setEditingLine((prev) => prev && { ...prev, showCompSearch: true }); }} className="text-[10px] text-emerald-400 hover:text-emerald-300 transition-colors">change product</button>
                                          )}
                                        </div>
                                        {!isEditingLine && onUpdatePoItem && (
                                          <button onClick={() => setEditingItem({ type: 'po', id: item.po_line_item_id })} title="Re-assign component" className="opacity-0 group-hover:opacity-100 flex-shrink-0 mt-0.5 p-0.5 text-slate-600 hover:text-blue-400 transition-all">
                                            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                          </button>
                                        )}
                                      </div>
                                  ...
                                  <td className="py-2 text-right">
                                    {isEditingLine ? (
                                      ...
                                      <div className="flex items-center justify-end gap-2">
                                        ...
                                        <div className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 flex-shrink-0 transition-opacity">
                                          {onUpdatePoLineItem && (
                                            <button onMouseDown={(e) => { e.preventDefault(); setEditingLine({ mode: 'edit', type: 'po', id: item.po_line_item_id, rowIdx: -1, targetId: 0, currency: item.currency, componentId: item.component_id ?? null, showCompSearch: false, qty: String(item.quantity), price: String(item.unit_cost), saving: false }); }} title="Edit qty & price" className="p-0.5 text-emerald-600 hover:text-emerald-400 hover:bg-emerald-500/10 rounded transition-all"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                                          )}
                                          {onDeletePoLineItem && (
                                            <button onClick={async (e) => { e.stopPropagation(); if (!window.confirm('Delete this PO line item?')) return; await onDeletePoLineItem(item.po_line_item_id); }} title="Delete" className="p-0.5 text-red-700 hover:text-red-400 hover:bg-red-500/10 rounded transition-all"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                          )}
                                        </div>
                                      </div>
                                    )}
