...
export default function ComponentEditor({ components, brandSuggestions, initialSearch = '', quoteItems = [], quotes = [], pos = [], poItems = [], suppliers = [], poCosts = [], componentHistory, competitorPrices, onSave, onAdd, onAddSupplier, onDelete, onSaveLineItem, onDeleteLineItem, onDeleteCompetitorPrice, onUpdateCompetitorPrice, componentLinks, onAddComponentLink, onDeleteComponentLink }: ComponentEditorProps) {
  ...
  // Pricing Mode lived here until 2026-08-29. Setting prices for 990 items is
  // its own job with its own question, and it moved whole to Catalog → Pricing
  // Tiers → Set Pricing, beside the tier ladder and margin bands that decide
  // the answer. This grid is about what an item IS, not what it sells for.
  ...
  const { quoteLinesByComponent, lastQuoteByComponent, sparklineLinesByComponent } = useMemo(() => {
    ...
    sparkRaw.forEach((entries, cid) => {
      const sorted = [...entries].sort((a, b) => {
        ...
        return d !== 0 ? d : a.quote_id - b.quote_id;
      });
  ...
  const handlePriceKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, componentId: string, fld = 'selling_price_idr') => {
    handleCellKeyDown(e as React.KeyboardEvent<HTMLInputElement>, componentId, 'selling_price_idr' as unknown as typeof NAV_FIELDS[number]);
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {filtered.map((c) => {
                ...
                const rowReadiness = specReadiness(c.category, (hasSpecs ? c.specifications : {}) as Record<string, unknown>);
                // Calculator categories always get the specs button (to add specs); others only once specs exist.
                const showSpecsButton = hasSpecs || rowReadiness.relevant;
                          ...
                          <button
                            ...
                            className={`px-2 py-1 text-xs font-semibold rounded-lg border transition-all ${
                              isSpecsOpen
                                ...
                                : rowReadiness.relevant
                                  ? rowReadiness.ready
                                    ? 'text-emerald-400 bg-transparent border-transparent hover:bg-emerald-500/10 hover:border-emerald-500/30'
                                    : 'text-amber-400 bg-transparent border-transparent hover:bg-amber-500/10 hover:border-amber-500/30'
                                  : 'text-slate-600 bg-transparent border-transparent hover:bg-slate-800/60 hover:border-slate-700/60 hover:text-sky-300'
                            }`}
                            title={isSpecsOpen ? 'Hide specifications'
                              : rowReadiness.relevant
                                ? rowReadiness.ready ? 'View specifications — calculator-ready' : `View specifications — missing for calculator: ${rowReadiness.missing.join(', ')}`
                                : 'View specifications'}
                          >
                            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                    ...
                    <tr className="bg-slate-900/40 border-t border-amber-500/10">
                      <td colSpan={visibleColCount} className="px-6 py-5">
                        <div className="mb-4 flex flex-wrap items-center gap-2">
                          {rowReadiness.relevant && (
                            <div className={`px-3 py-2 rounded-lg border text-xs font-semibold inline-flex items-center gap-2 ${
                              rowReadiness.ready
                              ...
                              {rowReadiness.ready ? (
                                <>
                                  ...
                                  Calculator-ready — carries every spec the {c.category?.replace(/_/g, ' ')} sizing engine needs
                                </>
                              ) : (
                                <>
                                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>
                                  Not calculator-ready — missing: {rowReadiness.missing.join(', ')}
                                </>
                              )}
                            </div>
                          )}
