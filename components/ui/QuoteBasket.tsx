...
export interface BasketItem extends QuoteLine {
  id: string;
  /** Which price was tapped (`''` = the plain sell price). */
  tierKey?: string;
}
...
export function useQuoteBasket(storeKey = 'icaproc.quotebasket') {
  ...
  return {
    ...
    /** Which price this product is currently in the list at, if any. */
    tierOf: (id: string): string | null => items.find((i) => i.id === id)?.tierKey ?? null,
    /**
     * Tapping a price is the whole interaction:
     *   • not in the list      → added at the price tapped
     *   • in it, same price    → removed
     *   • in it, another price → moved to the price just tapped
     * Each line therefore carries its OWN tier; a quote can mix them freely.
     * Returns what happened so the caller can say it.
     */
    tap: (item: BasketItem): 'added' | 'removed' | 'repriced' => {
      const at = items.findIndex((i) => i.id === item.id);
      if (at < 0) { persist([...items, item]); return 'added'; }
      if ((items[at].tierKey ?? '') === (item.tierKey ?? '')) {
        persist(items.filter((i) => i.id !== item.id));
        return 'removed';
      }
      persist(items.map((i) => (i.id === item.id ? { ...i, price: item.price, tier: item.tier, tierKey: item.tierKey } : i)));
      return 'repriced';
    },
            ...
            <div key={i.id} className="px-4 py-2.5 flex items-center gap-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[11px] text-slate-500 tabular-nums flex items-center gap-1.5">
                  ...
                  {i.tier && <span className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-400 text-[10px]">{i.tier}</span>}
