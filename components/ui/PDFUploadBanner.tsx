...
export default function PDFUploadBanner({ title, description, pdfData, uploading, onUpload, onClear }: Props) {
  return (
    <div className="mb-6 bg-slate-900/60 backdrop-blur-sm border border-emerald-500/25 rounded-2xl p-5 md:p-6 shadow-2xl ring-1 ring-white/5 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none" />
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
        <div className="flex items-start sm:items-center gap-4">
          <div className="p-2.5 bg-emerald-500/15 rounded-xl border border-emerald-500/30 shadow-inner flex-shrink-0">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <label className="cursor-pointer w-full sm:w-auto">
            ...
            <span className={`flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2 rounded-xl text-xs font-bold shadow-lg shadow-emerald-900/20 transition-all active:scale-[0.98] w-full sm:w-auto border border-emerald-500 ${uploading ? 'opacity-70 cursor-not-allowed' : ''}`}>
              {uploading
