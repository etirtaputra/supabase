 ...
 * X-axis: norm_value (capacity in canonical unit, e.g. Wp, W, Wh, mm²)
 * Y-axis: price per unit (TUC → last quote → last PO cost, all in IDR)
 * Quadrant lines at median X and median Y.
 * Log scale on X when range spans > 1 order of magnitude.
 */
'use client';
...
export default function CategoryPositioningMap({
  ...
  const toSvgX = (v: number): number => {
    if (useLogScale) {
      const lv = Math.log10(Math.max(v, xL));
      return PAD.left + ((lv - logXL) / (logXR - logXL)) * plotW;
    }
  ...
  const xTicks = useMemo(() => {
    if (!dots.length) return [];
    if (useLogScale) {
      const minPow = Math.floor(Math.log10(xL));
      const maxPow = Math.ceil(Math.log10(xR));
      const ticks: number[] = [];
      for (let p = minPow; p <= maxPow; p++) {
        for (const mult of [1, 2, 5]) {
          const t = mult * Math.pow(10, p);
          if (t > xL && t < xR) ticks.push(t);
        }
      }
      return ticks;
    }
        ...
        <div className="relative">
          ...
          <div className="overflow-x-auto">
            ...
            >
              ...
              {xTicks.map((t) => {
                ...
                return (
                  <g key={`xt-${t}`}>
                    ...
                    <text x={sx} y={PAD.top + plotH + 16} textAnchor="middle" fill="rgba(148,163,184,0.5)" fontSize="10">{fmtXTick(t)}</text>
                  </g>
                );
              })}
              ...
              {xMed > 0 && yMed > 0 && (
                <>
                  <text x={PAD.left + 8} y={PAD.top + 16} fill="rgba(251,146,60,0.55)" fontSize="9" fontWeight="600" letterSpacing="0.06em">PREMIUM SMALL</text>
                  <text x={xMedSvg + 8} y={PAD.top + 16} fill="rgba(168,85,247,0.55)" fontSize="9" fontWeight="600" letterSpacing="0.06em">PREMIUM</text>
                  <text x={PAD.left + 8} y={PAD.top + plotH - 8} fill="rgba(148,163,184,0.45)" fontSize="9" fontWeight="600" letterSpacing="0.06em">BUDGET</text>
                  <text x={xMedSvg + 8} y={PAD.top + plotH - 8} fill="rgba(52,211,153,0.6)" fontSize="9" fontWeight="600" letterSpacing="0.06em">BEST VALUE</text>
                </>
              )}
              ...
              {/* Axis labels */}
              <text x={PAD.left + plotW / 2} y={H - 10} textAnchor="middle" fill="rgba(148,163,184,0.5)" fontSize="11">
                {unitInfo?.axis ?? 'Capacity'}{useLogScale ? ' — log scale' : ''}
              </text>
              <text
                x={16}
                y={PAD.top + plotH / 2}
              ...
              {dots.map((d) => {
                ...
                // Truncate to 13 chars for inline label
                const shortLabel = d.label.length > 13 ? d.label.slice(0, 13) + '…' : d.label;
                // Flip label to the left when dot is near the right edge
                const nearRight = sx > PAD.left + plotW * 0.75;
                ...
                return (
                  <g key={d.component_id}>
                    ...
                    <circle
                      ...
                      stroke={isHovered ? '#fff' : 'rgba(15,23,42,0.6)'}
            ...
            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5">
              {[...brandColorMap.entries()].map(([brand, idx]) => (
                <div key={brand} className="flex items-center gap-1.5 text-[11px] text-slate-400">
                  <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: brandColor(brand, idx) }} />
                  {brand}
                </div>
              ))}
            </div>
